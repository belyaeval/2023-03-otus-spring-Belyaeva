...
public class AuthorServiceCommands {
    ...
    public String getAuthor(long id) {
        ...
        return author == null ? String.format("Author with id %d doesn't exist", id) :
                String.format("You got author № %d: %s", id, author.getName());
    }

    @ShellMethod(value = "Add author", key = {"aa", "create-a"})
    public String addAuthor(String aName) {
        ...
        return author == null ? String.format("Author '%s' is already exists", aName) :
                String.format("You inserted author № %d: %s", author.getId(), author.getName());
    }
}
